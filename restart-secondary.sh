#!/bin/bash
# ============================================================
# restart-secondary.sh — Recovery: pastikan gateway secondary jalan
# Dipanggil oleh sentinel Primary Hermes kalau secondary.heartbeat stale,
...
if [ -z "$SECONDARY_ALIVE" ]; then
    echo "$(date -u '+%Y-%m-%d %H:%M:%S UTC') restart-secondary: gateway secondary TIDAK jalan — starting..." >> "$LOG"
    ...
    if [ -n "$VERIFY" ]; then
        echo "$(date -u '+%Y-%m-%d %H:%M:%S UTC') restart-secondary: gateway secondary STARTED OK" >> "$LOG"
    else
        echo "$(date -u '+%Y-%m-%d %H:%M:%S UTC') restart-secondary: gateway secondary FAILED to start — see gateway.log" >> "$LOG"
    fi
else
    # Update heartbeat langsung (biar gak false-stale)
    date -u +%s > /root/hermes-shared/heartbeat/secondary.heartbeat 2>/dev/null
fi
